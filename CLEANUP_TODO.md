...
- [ ] Railway → Variables → either **delete `CORS_ORIGIN_REGEX`** entirely (if your
      ...
      ```
      CORS_ORIGIN_REGEX=^https://reco[-\w]*\.vercel\.app$
      ```
...
## 6. Optional: turn off `RECO_DEBUG` on Railway

`RECO_DEBUG=1` (or legacy `REMIP_DEBUG=1`) makes `/api/health` reveal extra config (CORS list, presence of
...
- [ ] Railway → Variables → delete `RECO_DEBUG` and `REMIP_DEBUG` (or set them to `0`).
